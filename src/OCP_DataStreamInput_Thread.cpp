...
//extern const wxEventType wxEVT_PI_OCPN_DATASTREAM;

#ifdef __WXMSW__
extern wxEventType wxEVT_PI_OCPN_DATASTREAM;
#else
extern wxEventType wxEVT_PI_OCPN_DATASTREAM;
#endif
...
PI_OCP_DataStreamInput_Thread::PI_OCP_DataStreamInput_Thread(ropeless_pi *Launcher,
...
PI_OCP_DataStreamInput_Thread::~PI_OCP_DataStreamInput_Thread(void)
{
    delete[] rx_buffer;
    delete[] temp_buf;
}

void PI_OCP_DataStreamInput_Thread::OnExit(void)
{
}
...
#ifdef __POSIX__
//    Entry Point
void *PI_OCP_DataStreamInput_Thread::Entry()
{
...
void PI_OCP_DataStreamInput_Thread::HandleASuccessfulRead( char *szBuf, int nread )
{
    if(nread > 0)
    {
...
void PI_OCP_DataStreamInput_Thread::Parse_And_Send_Posn(const char *buf)
{
    if( m_pMessageTarget ) {
        PI_OCPN_DataStreamEvent Nevent(wxEVT_PI_OCPN_DATASTREAM, 0);
...
void PI_OCP_DataStreamInput_Thread::ThreadMessage(const wxString &msg)
{
#if 0
...
bool PI_OCP_DataStreamInput_Thread::SetOutMsg(const wxString &msg)
{
...
int PI_OCP_DataStreamInput_Thread::OpenComPortPhysical(const wxString &com_name, int baud_rate)
{
...
int PI_OCP_DataStreamInput_Thread::CloseComPortPhysical(int fd)
{
    close(fd);
...
int PI_OCP_DataStreamInput_Thread::WriteComPortPhysical(int port_descriptor, const wxString& string)
{
    ssize_t status;
...
int PI_OCP_DataStreamInput_Thread::WriteComPortPhysical(int port_descriptor, char *msg)
{
    ssize_t status;
...
int PI_OCP_DataStreamInput_Thread::ReadComPortPhysical(int port_descriptor, int count, unsigned char *p)
{
//    Blocking, timeout protected read of one character at a time
//    Timeout value is set by c_cc[VTIME]
...
bool PI_OCP_DataStreamInput_Thread::CheckComPortPhysical(int port_descriptor)
{
    fd_set rec;
    struct timeval t;
...
//    Set up the serial port
    ...
    xcom_name = xcom_name.BeforeFirst(' ');               // strip off any description provided by Windows
