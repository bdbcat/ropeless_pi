...
void LATITUDE::Parse( int position_field_number, const SENTENCE& sentence )
{
   Set( sentence.Double( position_field_number ) );
}

void LATITUDE::Parse( double value )
{
   Northing = North;
   if(value < 0)
     Northing = South;
...
void LATITUDE::Set( double position )
{
//   assert( north_or_south != NULL );

   Latitude = abs(position);
